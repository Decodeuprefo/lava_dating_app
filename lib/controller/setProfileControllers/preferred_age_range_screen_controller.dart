...
class PreferredAgeRangeScreenController extends GetxController {
  final ApiController _apiController = Get.find<ApiController>();
  final ProfileModuleController _profileController = Get.find<ProfileModuleController>();
  bool isLoading = false;

  Future<void> updatePreferredAgeRange(BuildContext context) async {
    if (isLoading) return;
    ...
    try {
      final body = {
        'preferredAgeMin': _profileController.minAge.value,
        'preferredAgeMax': _profileController.maxAge.value,
      };
      ...
      try {
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          await StorageService.setProfileStep(12);
          Get.to(() => const SelectHeightScreen(), transition: Transition.noTransition);
        } else {
