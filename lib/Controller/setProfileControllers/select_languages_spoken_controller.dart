...
class SelectLanguagesSpokenController extends GetxController {
  final ApiController _apiController = Get.find<ApiController>();
  final ProfileModuleController _profileController = Get.find<ProfileModuleController>();
  bool isLoading = false;

  Future<void> updateLanguages(BuildContext context) async {
    if (isLoading) return;

    if (_profileController.selectedLanguages.isEmpty) {
      showSnackBar(context, 'Please select at least one language.', isErrorMessageDisplay: true);
      return;
    }
    ...
    try {
      final body = {
        'languagesSpoken': _profileController.selectedLanguages.toList(),
      };
      ...
      try {
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          await StorageService.setProfileStep(10);
          Get.to(() => const SelectRelationshipTypeScreen(), transition: Transition.noTransition);
        } else {
