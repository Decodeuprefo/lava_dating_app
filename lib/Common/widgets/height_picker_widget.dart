  ...
  void didUpdateWidget(HeightPickerWidget oldWidget) {
    ...
    if (oldWidget.isFtCm.value != widget.isFtCm.value) {
      if (widget.isFtCm.value) {
        ...
        int feetIndex = feetList.indexOf(widget.minHeight);
        int inchesIndex = inchesList.indexOf(widget.maxHeight);
        
        // If feet value not found (e.g., coming from cm mode), default to 5
        if (feetIndex < 0) {
          feetIndex = feetList.indexOf(5);
          if (feetIndex < 0) feetIndex = 1; // 5 is typically at index 1 (4, 5, 6, 7)
          // Update the value through callback
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (feetIndex >= 0 && feetIndex < feetList.length) {
              widget.onMinHeightChanged(feetList[feetIndex]);
            }
          });
        }
        
        // If inches value not found, default to 0
        if (inchesIndex < 0) {
          inchesIndex = 0;
          WidgetsBinding.instance.addPostFrameCallback((_) {
            widget.onMaxHeightChanged(0);
          });
        }
        
        // Use post-frame callback to ensure controller is ready
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (feetIndex >= 0 && _feetController.hasClients && feetIndex < feetList.length) {
            _feetController.jumpToItem(feetIndex);
          }
          if (inchesIndex >= 0 && _inchesController.hasClients && inchesIndex < inchesList.length) {
    ...
    if (widget.isFtCm.value) {
      ...
      // Centimeter mode
      if (oldWidget.minHeight != widget.minHeight ||
          oldWidget.isFtCm.value != widget.isFtCm.value) {
        ...
        return SingleChildScrollView(
          child: Column(
            children: [
              Row(
                mainAxisAlignment:
                    widget.isFtCm.value ? MainAxisAlignment.spaceBetween : MainAxisAlignment.center,
                children: [
                  Text(
